...
class Ecarter(Phase):
    ...
    def message(self):
        contrat,preneur =  self.partie.get_contrat()
        message = "{{Joueur[{}]}} doit maintenant faire son écart.".format(preneur)
        return message   
    
    def choix(self):
        return {self.prompt() :[("ECARTER","Mettre à l'écart.")]} #.format(cards_as_string(self.partie.mains[joueur],triees=True))}
